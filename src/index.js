...
var fs = require('fs');
    var barcode = {
        ...
        digitToBmpRenderer: function(settings, digit, hri, mw, mh, callback){
            ...
            this.write(bmp, 'bmp', callback);
            //this.resize($container, mw * columns + padding).append(object);

        },
        // bmp 1D barcode renderer
        digitToBmp: function(settings, digit, hri, callback){
            var w = barcode.intval(settings.barWidth);
            var h = barcode.intval(settings.barHeight);
            this.digitToBmpRenderer(settings, this.bitStringTo2DArray(digit), hri, w, h, callback);
        },
        // bmp 2D barcode renderer
        digitToBmp2D: function(settings, digit, hri, callback){
            var s = barcode.intval(settings.moduleSize);
            this.digitToBmpRenderer(settings, digit, hri, s, s, callback);
        },
        ...
        write: function(data, type, callback){
            fs.writeFile('barcode.'+type,data,null,function(err){
                var result = true;
                if(err){
                    console.log(err);
                    result = false;
                }
                console.log('===svg file create==');
